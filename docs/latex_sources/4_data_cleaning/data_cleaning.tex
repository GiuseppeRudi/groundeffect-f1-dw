...
\begin{document}
	...
	The focused missing value script produces row-level flags for the \texttt{lap} and \texttt{result} tables.
	...
	Expected nulls with \texttt{missing\_information\_area = NONE} are not treated as cleaning problems.
	They are useful for documentation, but they do not require a transformation of the cleaned table.
	...
	\begin{longtable}{p{4cm}p{4cm}p{3cm}p{4cm}}
		...
		& \texttt{ADD\_FLAG}
		& The row is preserved, but tyre-based analysis must know that tyre information is incomplete. \\
		\midrule
		
		\texttt{SECTOR\_INFORMATION}
		& One or more sector times are missing
		& \texttt{ADD\_FLAG}
		& The row may still be usable for total lap time analysis, but not for complete sector analysis. \\
		\midrule
		
		\texttt{SPEED\_INFORMATION}
		& One or more speed values are missing
		& \texttt{ADD\_FLAG}
		& The row may still be usable for lap time analysis, but not for speed-based analysis. \\
		\midrule
		
		\texttt{TRACK\_STATUS\_INFORMATION}
		& \texttt{track\_status} is missing or not interpretable
		& \texttt{ADD\_FLAG}
		& The row is preserved, but clean-lap filtering and track-context analysis are limited. \\
		\midrule
		
		\texttt{WEATHER\_INFORMATION}
		& Lap-level weather context is missing
		& \texttt{ADD\_FLAG}
		& The row is preserved, but weather-based comparison cannot safely use it. \\
		\midrule
		
		\texttt{PIT\_INFORMATION}
		& Pit interpretation is incomplete
		& \texttt{ADD\_FLAG}
		& The row is preserved, but special-lap interpretation is limited. \\
		\midrule
	...
	When the action is \texttt{ADD\_FLAG}, the value of \texttt{missing\_information\_area} is added to the row-level quality metadata of the cleaned \texttt{lap} table.
	
	For example, if a lap has a missing \texttt{sector2\_time\_ms}, the row is preserved and the cleaned table records that the row has missing \texttt{SECTOR\_INFORMATION}.
	
	\subsection{Missing Value Actions for the Result Table}
	
	\begin{longtable}{p{5cm}p{4cm}p{2.5cm}p{4cm}}
		\toprule
		\textbf{Missing Information} & \textbf{Condition} & \textbf{Action} & \textbf{Reason} \\
		\midrule
		\endfirsthead
		...
		& position or classified\_position is missing when required
		& \texttt{DROP\_ROW}
		...
		& \texttt{ADD\_FLAG}
		& The row is preserved, but qualifying performance analysis must know that qualifying information is incomplete. \\
		\midrule
		
		\texttt{RACE\_CONTEXT\_INFORMATION}
		& Race context attributes are missing
		& \texttt{ADD\_FLAG}
		& The row is preserved, but race-context analysis is limited. \\
		\midrule
	...
	This means that missing classification information is handled more strictly than other missing result information.
	If classification information is missing, the row is removed from the cleaned result table.
	If qualifying or race context information is missing, the row is preserved with a flag.
	...
	The cleaning script does not automatically replace outlier values.
	The action depends on the metric and on the consensus score.
	...
	\begin{longtable}{p{4cm}p{3cm}p{3cm}p{5cm}}
		...
		& 1 or 2
		& \texttt{ADD\_FLAG}
		& The row is preserved, but sector-based analysis should know that sector information may be unreliable. \\
		\midrule
		
		Speed metrics
		& 1 or 2
		& \texttt{ADD\_FLAG}
		& The row is preserved, but speed-based analysis should know that speed information may be unreliable. \\
		\bottomrule
	\end{longtable}
	...
	They only add flags, because the same lap may still be useful for total lap time analysis.
	
	For example, if \texttt{speed\_i1} is detected as an outlier, the cleaned \texttt{lap} row can still be loaded for lap time analysis, but it should be excluded from speed-based visualizations.
	...
	\begin{longtable}{p{4.5cm}p{10cm}}
		...
		& Column affected by the action, when the action concerns a specific attribute. \\
		\midrule
		...
		& Type of analytical information missing from the row, when the action comes from the focused missing value script. \\
		\midrule
		...
		\texttt{old\_value}
		& Value before cleaning, only when a value is modified. \\
		\midrule
		
		\texttt{new\_value}
		& Value after cleaning, only when a value is modified. \\
		\midrule
