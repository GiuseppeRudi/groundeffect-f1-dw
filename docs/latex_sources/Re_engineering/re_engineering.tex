...
\begin{document}
	...
	\begin{table}[H]
		\centering
		\caption{Attributes of the current \texttt{} table}
	...
	The \texttt{session} table is identified by the suggorate primary key \texttt{session\_id)}.
	...
	\begin{table}[H]
		...
		\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{3.6cm} >{\raggedright\arraybackslash}p{3.2cm} >{\raggedright\arraybackslash}X}
			...
			\addlinespace
			Create suggorate primary keys & \texttt{Grand Prix} and \texttt{Session} & To uniquely identify rows at the correct granularity \\
			\addlinespace
	...
	\begin{table}[H]
		...
		\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{4.1cm} >{\raggedright\arraybackslash}p{3.2cm} >{\raggedright\arraybackslash}X}
			...
			\textbf{Operation} & \textbf{Applied to} & \textbf{Purpose} \\
			\midrule
			Addition of \texttt{SeasonYear} & Raw results & To connect each result to the corresponding session \\
			\addlinespace
			...
			\addlinespace
			Create suggorate primary key & Final \texttt{result} table & To uniquely identify the final result \\
			\addlinespace
	...
	\begin{table}[H]
		...
		\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{4.1cm} >{\raggedright\arraybackslash}p{3.2cm} >{\raggedright\arraybackslash}X}
			...
			Renaming of converted columns with the suffix \texttt{\_ms} & Time-related attributes & To make the unit of measure explicit and improve schema readability \\
			\addlinespace
			Creation of suggorate primary key & Final \texttt{lap} table & To uniquely identify each lap in a specific session \\
			\addlinespace
	...
	\begin{table}[H]
		...
		\begin{tabularx}{0.95\textwidth}{>{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X}
			...
			\texttt{wind\_spee  }  & \texttt{pressure}        & \texttt{wind\_direction} \\
			\texttt{time\_ms}       &     & \\
			\bottomrule
		\end{tabularx}
	\end{table}
	
	From a relational point of view, the \texttt{weather} table is identified by the suggorate primary key \texttt{weather\_id}.
	...
	Therefore, the attribute \texttt{season\_id} was added in order to integrate the raw structure into the reconciled schema.
	...
	\begin{figure}[H]
		...
		\caption{Raw source tables extracted from FastF1 and Ergast.}
	\end{figure}
