...
\begin{document}
	...
	In the reconciled schema, \texttt{lap\_id} and \texttt{result\_id} identify source
	records and support lineage and data-quality checks. They are not exported as analytical
	fact columns. The physical warehouse facts use \texttt{lap\_performance\_key} and
	\texttt{session\_result\_key} as their primary keys.
	...
	The reconciled schema remains normalized and contains \texttt{season}, \texttt{circuit},
	\texttt{grand\_prix}, \texttt{session}, \texttt{driver}, \texttt{team},
	\texttt{result}, \texttt{lap}, \texttt{weather}, and \texttt{track\_status}. The
	denormalization described later applies only to the physical warehouse.

	The final physical warehouse contains exactly the following tables:
	\begin{itemize}
		\item \texttt{fact\_lap\_performance} and \texttt{fact\_session\_result};
		\item \texttt{dim\_driver}, \texttt{dim\_team}, and \texttt{dim\_grand\_prix};
		\item \texttt{dim\_tyre\_context}, \texttt{dim\_lap\_weather\_context}, and
		\texttt{dim\_lap\_data\_quality};
		\item \texttt{dim\_result\_outcome}, \texttt{dim\_result\_weather\_context}, and
		\texttt{dim\_result\_data\_quality}.
	\end{itemize}
	No physical \texttt{dim\_circuit}, \texttt{dim\_season}, \texttt{dim\_session\_type},
	\texttt{dim\_lap\_type}, \texttt{dim\_track\_status\_context}, or generic
	\texttt{dim\_weather\_context} is created.
	...
	Operationally, this transformation is carried out during the ETL process.
	The ETL uses \texttt{lap\_start\_time\_ms} to associate each lap with the most recent
	weather observation and track-status record in the same session. A backward temporal
	association is attempted first; when no preceding observation exists, the nearest record
	in that same session is used as fallback. Session boundaries are never crossed.
	In this way, session-level temporal data are transformed into lap-level attributes.
	
	Derived attributes are:
	\begin{itemize}
		\item \textit{AirTempClass}
		\item \textit{TrackTempClass}
		\item \textit{RainFlag}
		\item \textit{WindSpeedClass}
	...
	\begin{itemize}
		...
		\item Only \textbf{SeasonYear} is retained as a level of the Grand Prix hierarchy.
		The championship descriptors are not required by either analytical fact.
	...
	\begin{longtable}{|p{4.4cm}|p{10.8cm}|}
		...
		ChampionDriver & Removed because it describes the final championship outcome rather than the grain of either fact. \\ \hline

		ChampionTeam & Removed because it describes the final championship outcome rather than the grain of either fact. \\ \hline
	...
	The same pruning choice applies to the \textit{Session Result} fact. Seasonal analysis is
	still supported through \texttt{season\_year}, which is retained in the Grand Prix hierarchy
	and physically stored in \texttt{dim\_grand\_prix}.
	...
	groups tyre usage into the following closed classes:
	
	\begin{itemize}
		\item \textit{Low}: $0 \leq \texttt{tyre\_life} \leq 5$;
		\item \textit{Medium}: $5 < \texttt{tyre\_life} \leq 15$;
		\item \textit{High}: $\texttt{tyre\_life} > 15$;
		\item \textit{Unknown}: value missing, negative, or unreliable.
	\end{itemize}
	...
	Driver age is computed with the fixed reference date 1 January 2022 as
	$\left\lfloor(\texttt{2022-01-01}-\texttt{date\_of\_birth})/365.2425\right\rfloor$.
	Possible values are:
	
	\begin{itemize}
		\item \textit{Young}: age below 25;
		\item \textit{Intermediate}: age from 25 to 29;
		\item \textit{Senior}: age at least 30;
		\item \textit{Unknown}: birth date missing or unreliable.
	\end{itemize}
	...
	% ------------------------------------------------------------
	\subsubsection{Derived Quality Attributes from Data Cleaning}
	% ------------------------------------------------------------
	
	Some additional lap-level attributes are introduced after the Data Cleaning phase.
	
	These attributes are not part of the original reconciled schema and they are not included in
	the initial General Attribute Tree. They are derived from the outputs of the focused
	Missing Values analysis, the Outlier Detection script, and the Data Cleaning workflow.
	
	Their purpose is not to describe a Formula 1 sporting property directly, but to describe the
	analytical usability of each lap.
	
	For this reason, they are considered \textit{derived quality attributes}.
	
	The Data Cleaning script creates one boolean flag for each relevant analytical information
	area. A value equal to \texttt{true} means that the corresponding information area is missing,
	incomplete, or unreliable for that lap. A value equal to \texttt{false} means that no issue was
	detected for that information area.
	

	
	These attributes are attached to \texttt{Lap\_Id}, because each flag describes the quality
	condition of one specific lap row.
	
	\begin{longtable}{|p{5.2cm}|p{8.8cm}|}
		\hline
		\textbf{Derived Quality Attribute} & \textbf{Analytical Meaning} \\
		\hline
		\endfirsthead
		...
		\textit{HasSectorInformationIssue}
		&
		Indicates that one or more sector-time attributes are missing, invalid, inconsistent, or detected as unreliable. The lap should not be used for complete sector analysis.
		\\
		\hline
		
		\textit{HasSpeedInformationIssue}
		&
		Indicates that one or more speed attributes are missing, invalid, or detected as unreliable. The lap should not be used for speed-based analysis.
		\\
		\hline
		
		\textit{HasTyreInformationIssue}
		&
		Indicates that tyre-related information, such as compound, stint, or tyre life, is missing or unreliable. The lap should be used carefully in tyre-based comparisons.
		\\
		\hline
		
		\textit{HasWeatherInformationIssue}
		&
		Indicates that lap-level weather context is missing or unreliable. The lap should not be used for weather-based comparison without caution.
		\\
		\hline
		
		\textit{HasTrackStatusInformationIssue}
		&
		Indicates that track-status information is missing or not interpretable. This affects clean-lap filtering and track-condition analysis.
		\\
		\hline
		
		\textit{HasPitInformationIssue}
		&
		Indicates that pit-related information is incomplete. This affects the interpretation of normal laps, in-laps, out-laps, and pit-affected laps.
		\\
		\hline
	...
	\begin{longtable}{|p{5.5cm}|p{4cm}|p{3.2cm}|}
		\hline
	...
	\begin{longtable}{|p{2.7cm}|p{5.3cm}|p{6.8cm}|}
		\hline
		...
		\textit{Driver}
		& \texttt{Driver\_Id}, \textit{FirstName}, \textit{LastName}, \textit{Abbreviation}, \textit{Nationality}, \textit{DriverAgeClass}
		...
		\textit{EventFormat}, \textit{SeasonYear}, \textit{CircuitId}, \textit{CircuitName},
		\textit{Country}, \textit{Location}, \textit{GlobalCircuitCategory},
		\textit{Sector1Category}, \textit{Sector2Category}, \textit{Sector3Category}
		& This dimension allows the analysis of lap performance by grand prix, season, calendar
		order, and weekend format. \\
		\hline
		
		\textit{Circuit hierarchy level}
		...
		& This is a conceptual hierarchy in the DFM. The attributes are physically stored
		inside \texttt{dim\_grand\_prix}; no \texttt{dim\_circuit} table is created. \\
		\hline
		
		\textit{Session Type (DD)}
		& \textit{SessionType}
		...
		\textit{Track Status (DD)}
		&  \textit{TrackStatusCategory}
		& This dimension is mainly used for filtering the track condition. \\
		\hline
		
		\textit{Lap Weather Context}
		& \textit{RainFlag}, \textit{AirTempClass}, \textit{TrackTempClass}, \textit{WindSpeedClass}
		& Physical dimension \texttt{dim\_lap\_weather\_context}, referenced through
		\texttt{lap\_weather\_context\_key}. \\
		\hline
		
		\textit{Lap Data Quality}
		& \textit{HasSectorInformationIssue}, \textit{HasSpeedInformationIssue},
		\textit{HasTyreInformationIssue}, \textit{HasWeatherInformationIssue},
		\textit{HasTrackStatusInformationIssue}, \textit{HasPitInformationIssue}
		& This dimension groups the boolean quality flags derived during the Data Cleaning phase.
		It is used to filter laps according to the type of information that is missing or unreliable.
		For example, laps with \textit{HasSpeedInformationIssue} equal to true should not be used for speed-based analysis. \\
		\hline
	...
	The entries marked \texttt{DD} are degenerate dimensions stored directly in
	\texttt{fact\_lap\_performance}. They do not generate separate physical tables.
	
	The \textit{Lap Data Quality} dimension is introduced as a technical quality dimension.
	It does not describe the sporting context of the lap, but the reliability and completeness
	of the information available for that lap.
	
	The attributes of this dimension are boolean flags derived during the Data Cleaning phase.
	They indicate whether a specific analytical area is affected by missing, invalid, or
	outlier information.
	
	This dimension can be interpreted as a \textit{junk dimension} in the Star Schema.
	A junk dimension groups together several low-cardinality technical flags that are useful
	for filtering and data selection, but that do not justify the creation of many separate
	dimensions.

	Some dimensions, such as \textit{Tyre Context} and \textit{Lap Weather Context}, are created by
	grouping several related attributes into a single analytical dimension.
	
	This choice is motivated by the fact that these attributes describe the same contextual
	aspect of the lap. \textit{Tyre Context} describes the tyre condition, while
	\textit{Lap Weather Context} describes the aligned environmental condition.
	
	Therefore, they are not direct copies of single entities in the reconciled schema, but
	semantic dimensions created after pruning, grafting, and derived-attribute definition.
	This makes the dimensional model clearer and more useful for OLAP queries, because related
	attributes are analyzed together instead of being scattered across the fact table or split
	into many small dimensions.
	...
	% ------------------------------------------------------------
	\subsubsection{Data-Cleaning-Derived Quality Attributes}
	% ------------------------------------------------------------
	
	After the Data Cleaning phase, some additional boolean attributes are introduced for the
	\textit{Session Result} fact.
	
	These attributes are not taken directly from the original \textit{Result} relation.
	They are derived from the focused missing value analysis and from some cleaning decisions
	applied to the \texttt{result} table.
	
	Their purpose is not to measure performance.
	Instead, they describe whether some analytical information associated with a result row is
	missing, incomplete, or unreliable.
	
	For this reason, they are treated as quality descriptors of the result row.
	
	The information area \texttt{CLASSIFICATION\_INFORMATION} is handled differently.
	If classification information is missing when required, the corresponding result row is
	removed during cleaning, because it cannot support the grain of the \textit{Session Result}
	fact.
	
	Therefore, no boolean flag is created for classification information in the cleaned
	\texttt{result} table.
	
	The retained data-cleaning-derived attributes are:
	
	\begin{longtable}{|p{5.5cm}|p{10.3cm}|}
		\hline
		\textbf{Derived Quality Attribute} & \textbf{Analytical Meaning} \\
		\hline
		\endfirsthead
		...
		\textit{HasQualifyingInformationIssue}
		&
		Indicates that qualifying-related information is missing, incomplete, or suspicious.
		The row should not be used for complete qualifying time analysis without filtering.
		\\
		\hline
		
		\textit{HasRaceContextInformationIssue}
		&
		Indicates that some race-context information is missing, invalid, or unreliable.
		The row may still be useful for classification analysis, but not for all race-context
		analyses.
		\\
		\hline
	...
	\begin{longtable}{|p{5.5cm}|p{5.4cm}|p{5.5cm}|}
		\hline
		...
		\textit{HasQualifyingInformationIssue}
		& Data Cleaning output on \texttt{result}
		& Quality flag / Result Quality descriptor \\
		\hline
	...
	\begin{longtable}{|p{3cm}|p{5.3cm}|p{6.5cm}|}
		\hline
		\textbf{Dimension} & \textbf{Attributes} & \textbf{Motivation} \\
		\hline
		\endfirsthead
		...
		\textit{GlobalCircuitCategory}, \textit{Sector1Category}, \textit{Sector2Category},
		\textit{Sector3Category}
		& Conceptual DFM hierarchy stored physically inside \texttt{dim\_grand\_prix};
		no separate circuit dimension is created. \\
		\hline
		...
		\textit{Result Data Quality}
		& \textit{HasQualifyingInformationIssue},
		\textit{HasRaceContextInformationIssue}
		& This technical junk dimension groups boolean quality flags derived from the
		Data Cleaning phase. It is used to filter result rows according to the
		reliability of qualifying and race-context information. \\
		\hline
		
	\end{longtable}

	The entry marked \texttt{DD} is stored directly as \texttt{session\_type} in
	\texttt{fact\_session\_result}; it does not generate a physical session-type dimension.
	The categorical attributes of Result Weather Context are stored in
	\texttt{dim\_result\_weather\_context} and referenced through
	\texttt{result\_weather\_context\_key}.
	
	
	In addition to the main business dimensions, the model includes a technical
	\textit{Result Data Quality} dimension.
	
	This dimension is a junk dimension because it groups low-cardinality boolean
	quality flags derived from the Data Cleaning phase. These flags are not measures
	and they do not represent a real business entity. They are included to support
	OLAP filtering and to avoid using result rows in analyses for which some
	important information is missing or unreliable.
