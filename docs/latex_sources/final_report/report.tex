...
\begin{document}
	...
	More detailed documentation for each stage is provided in the project repository.
	...
	The interest is therefore not only in absolute performance, but also in the relative changes that emerged across sessions, circuits, sectors, tyre conditions, and race contexts.
	
	This report focuses on the design and engineering choicesn, while the final visual exploration is developed separately in Tableau.
	...
	\begin{figure}[H]
		...
		\caption{Conceptual representation of the 2022 regulatory change in Formula 1 car aerodynamics.}
	...
	Each of them focuses on a different analytical perspective, so that the final interpretation can be built by combining multiple pieces of evidence:
	...
	\begin{figure}[H]
		...
		\caption{Project-specific roadmap from Formula 1 data sources to the final Data Warehouse.}
	...
	Additional analytical information not directly available from the APIs was introduced through manually built domain tables, supported by domain knowledge and external descriptive sources. 
	...
	\begin{table}[H]
		...
		\begin{tabularx}{\linewidth}{p{3.6cm}X}
			\toprule
			\textbf{Source} & \textbf{Information extracted or introduced} \\
			\toprule
			FastF1 API 
	...
	For this reason, the re-engineering phase transformed the extracted data into a coherent relational structure with explicit keys, clearer granularities, standardized names, and meaningful links between tables.
	
	Only the most relevant re-engineering decisions are reported in this section. 
	The complete re-engineered table structures and the complete logical schema are reported in Appendix~\ref{appendix:reengineered-tables} and Appendix~\ref{appendix:logical-schema}.
	...
	Their attributes are shown in Figure~\ref{fig:domain-tables}, while the external sources used to populate and validate them are reported in the references.
	...
	This organization is not suitable for a relational schema because it mixes two different semantic levels.
	
	For this reason, the raw schedule was split into two relations: \texttt{grand\_prix} and \texttt{session}. 
	...
	This split makes the reconciled schema closer to the real Formula 1 structure: one season contains several Grand Prix weekends, and each Grand Prix contains multiple sessions.
	...
	For this reason, lap records were enriched by matching them with the corresponding session result information.
	
	A separate \texttt{driver\_team\_season} association was also considered but not materialized. 
	...
	\begin{longtable}{p{4.2cm}p{\dimexpr\linewidth-4.2cm-4\tabcolsep\relax}}
		...
		& Surrogate primary keys were introduced to provide stable row identifiers, while foreign keys were used to make table relationships explicit and enforceable. \\
		\midrule
		
		Time conversion to \texttt{ms}
		& Time-related values returned by the APIs as timedelta objects were converted into integer milliseconds with the suffix \texttt{\_ms} to simplify numerical comparison. \\
		\midrule
		
		Removal of duplicated descriptors
		& Repeated driver, team, event, or session descriptors were removed when the same information could be recovered through joins with reference tables. \\
	...
	For readability, the E/R diagram in Figure~\ref{fig:reconciled-er-diagram} reports only the most relevant attributes. The complete logical schema is reported in Appendix~\ref{appendix:logical-schema}.
	...
	In this project, however, the source data are mainly obtained from API-based Python structures returned by FastF1. Therefore, the input is not an uncontrolled textual source: each column already has an expected computational representation.
	
	For this reason, the Data Quality Assessment does not focus on generic type-parseability checks. Instead, it focuses on quality problems that can still exist after typed loading.
	
	\subsection{General Data Quality Assessment}
	
	The General Data Quality Assessment is applied to the reconciled schema in order to evaluate its structural and semantic quality, producing deterministic evidence for the following cleaning workflow without modifying the data.
	
	
	The assessment is organized according to the following data quality dimensions.
	...
	Since the complete set of rules is too detailed for the main report, this section presents only the most relevant assessment choices. A summarized dimension-by-dimension view of the applied rules is reported in Appendix~\ref{appendix:dqa-rules}.
	...
	\begin{table}[H]
		...
		\begin{tabularx}{\linewidth}{p{5cm}X}
			...
			& Verifies that the sum of the three sector times is coherent with the total lap time, allowing a small tolerance for source precision. Pit-in and pit-out laps are excluded from this rule because they may distort normal lap-time interpretation. \\
			...
			& Verifies that each numerical track-status code always corresponds to the expected textual message. For example, the same status code must always be associated with the same meaning, such as \texttt{0} with \texttt{AllClear}. \\
			...
			& Verifies that circuit and sector classification attributes are present, since they are required to compare performance across circuit types. \\
	...
	Therefore, missing values in the qualifying attributes are classified as \texttt{EXPLAINED NULL}, with no missing analytical information. 
	...
	A score of 0 means that no method flags the value, a score of 1 indicates a weak anomaly flagged by only one method, and a score of 2 identifies a stronger suspicious value flagged by both methods.
	...
	These plots are reported before the cleaning phase because they describe the detected issues, while the following section explains how these issues are treated.
	
	\begin{figure}[H]
		\centering
		\figplaceholder{
			General Data Quality Assessment score heatmap\\
			Quality dimensions by table
		}
	...
	The general assessment was intentionally based on deterministic and structural checks, such as completeness of key attributes, uniqueness, validity, consistency, and referential integrity. 
	More domain-specific issues, especially missing values and numerical anomalies, are analysed separately through the focused missing-value interpretation and outlier detection procedures.
	...
	\begin{figure}[H]
		...
		\begin{subfigure}{0.48\linewidth}
			\centering
			\figplaceholder{
				Missing-value class distribution\\
				Explained nulls vs suspicious nulls
			}
			\caption{Missing-value classes}
		...
		\begin{subfigure}{0.48\linewidth}
			\centering
			\figplaceholder{
				Missing values by information area\\
				Speed, sector, lap time, tyre, race context
			}
			\caption{Missing values by information area}
			\label{fig:missing-information-area}
		\end{subfigure}
		\caption{Visual summary of the focused missing-value interpretation.}
		\label{fig:missing-values-summary}
	\end{figure}
	
	The missing-value plots show that a relevant part of the null values is explained by the Formula One domain. 
	Some attributes are not applicable in every context: for example, qualifying-related values are not expected in race-only situations, and race-related values are not always meaningful in qualifying sessions. 
	For this reason, explained nulls are separated from suspicious nulls instead of treating all missing values as errors.
	
	The distribution by information area shows that missing values are mainly concentrated in speed, sector, and lap-time information. 
	...
	\begin{figure}[H]
		\centering
		\figplaceholder{
			Outlier detection methods by metric\\
			IQR, Modified Z-score, and consensus outliers
		}
		\caption{Outlier detection methods by metric.}
		\label{fig:outlier-methods-by-metric}
	\end{figure}
	
	The outlier plot compares the number of values flagged by the IQR method, the Modified Z-score, and the final consensus rule. 
	The presence of many outliers is expected in Formula One data, because lap times, sector times, and speed values can vary significantly depending on traffic, pit events, tyres, weather, yellow flags, Safety Car phases, and race situations. 
	For this reason, outliers are not automatically interpreted as wrong values.
	
	The consensus rule makes the detection more conservative: a value is considered more suspicious only when both methods agree. 
	This reduces the risk of removing valid but unusual racing situations and supports the cleaning strategy adopted in the project, where only strong anomalies on key measures are removed, while other suspicious cases are mainly preserved with quality flags.
	...
	These rules are safe because they standardize representation without changing the semantic meaning of the data.
	...
	Only the most representative policies are reported in the main text. The complete DQA-driven cleaning rules are reported in Appendix~\ref{appendix:dqa-cleaning-rules}.
	...
	\begin{table}[H]
		...
		\begin{tabular}{
				...
				>{\raggedright\arraybackslash}m{7.1cm}
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.90\linewidth]{figures/season-grandprix-session.pdf}
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.75\linewidth]{figures/driver-team.pdf}
	...
	\begin{figure}[H]
		\centering
		 \includegraphics[width=1\linewidth]{figures/lap_focus.pdf}
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.60\linewidth]{figures/result_focus.pdf}
	...
	\begin{table}[H]
		...
		\begin{tabular}{@{}c l !{\vrule width 0.5pt} c l !{\vrule width 0.5pt} c l !{\vrule width 0.5pt} c l@{}}
			\toprule
			\legendbox{directdimensioncolor} & Direct dimension
			& \legendbox{measurecolor} & Measure
			& \outlinedlegendbox{contextualboxcolor}{dashed} & Contextual/composite dimension
			& \outlinedlegendbox{junkboxcolor}{densely dotted} & Junk dimension \\
			\bottomrule
		\end{tabular}
	\end{table}
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.70\linewidth]{figures/lap_fact_schema.pdf}
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.70\linewidth]{figures/result_fact_schema.pdf}
	...
	This appendix contains additional technical material related to the re-engineering phase. 
	...
	Figure~\ref{fig:appendix-raw-api-structures} reports the complete raw API structures used as input for the re-engineering phase. 
	
	\vspace{5em}
	\begin{figure}[H]
		\centering
		% Replace with the complete Draw.io export
	...
	Figure~\ref{fig:appendix-logical-schema} reports the complete logical schema of the reconciled PostgreSQL database. 
	This figure provides the full technical view of the schema, including detailed table structures and relationships.
	...
	\subsection{Additional Lap-table Missing Rules}
