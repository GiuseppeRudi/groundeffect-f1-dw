...
\newcommand{\customtitle}{%
	\begin{center}
		...
		\begin{center}
			\raisebox{0.20cm}{\includegraphics[height=0.70cm]{figures/f1_logo.png}}
			\hspace{0.75cm}
			\textcolor{gray!60}{\rule{0.6pt}{1.20cm}}
			\hspace{0.75cm}
			\raisebox{-0.15cm}{\includegraphics[height=1.30cm]{figures/unical_logo.png}}
		\end{center}
		...
		{\large A Data Warehouse for Formula 1 Performance Analysis\par}
		
		\vspace{0.10cm}
		
		{\normalsize\textit{2021--2022 Seasons}\par}
		\vspace{0.35cm}
...
\begin{document}
	...
	More detailed documentation for each stage is provided in the
	\hyperref[sec:repository-material]{Supplementary Material}.
	...
	This comparison is particularly meaningful because the 2022 season introduced a new technical framework centred on ground-effect aerodynamics, which led to a substantial shift in car design philosophy. \cite{fia2022TechnicalRegulations,f12021vs2022}
	...
	\begin{figure}[H]
		...
		\caption{Conceptual representation of the 2022 regulatory change in car aerodynamics. Source: \cite{therace}}
	...
	This question is then divided into more business questions. 
	Each of them focuses on a different analytical perspective, so that the final interpretation can be built by combining the following perspectives:
	...
	\begin{table}[H]
		...
		\begin{tabularx}{\linewidth}{p{5cm}X}
			...
			\midrule
			Ergast/Jolpica API~\cite{jolpicaDocs} 
	...
	The objective is to show that these information was not directly database-friendly. 
	The complete raw structures, including all original attributes, are reported in Appendix~\ref{appendix:raw-api-structures}.
	
	\begin{figure}[H]
		\centering

		\includegraphics[width=0.80\linewidth]{figures/raw_tables_overview.pdf}
	...
	\paragraph{Circuit integration and Grand Prix matching.}
	
	Since the \texttt{circuit} table was introduced as a domain relation, it was necessary to link each Grand Prix to its corresponding circuit. The \texttt{grand\_prix} table was therefore enriched with the foreign key \texttt{circuit\_id} through a semi-automatic matching procedure based on the combination of \texttt{country} and \texttt{location}, followed by manual validation. This established the relationship between Grand Prix events and circuits in the reconciled schema.
	...
	\begin{longtable}{p{4.2cm}p{\dimexpr\linewidth-4.2cm-4\tabcolsep\relax}}
		...
		& Repeated driver, team, event, or session were removed when the same information could be recovered through joins with reference tables. \\
	...
	\begin{figure}[H]
		\centering
		 \includegraphics[width=0.90\linewidth]{figures/e-r.pdf}
	...
	The Data Quality Assessment phase also produced visual outputs to summarize the evidence.
	...
	The missing-value plots show that a relevant part of the null values is explained by the Formula 1 domain. The distribution by information area shows that missing values are mainly concentrated in speed, sector, and lap-time information. 
	...
	The comparison shows that the consensus rule identifies fewer strong anomalies than either individual method, particularly the IQR method.
	...
	\begin{table}[H]
		...
		\begin{tabular}{
			...
			lap
			& Missing structural identifiers.
			& DROP ROW \\
	...
	For the Driver branch, the permanent number are pruned because they are not analytically significant. 
	...
	The team and driver URLs are pruned because it is only an external reference. 
	...
	The \texttt{weather} and \texttt{track\_status} tables require a
	separate treatment because they are not static descriptors of a
	session. Both tables contain the attribute \texttt{time\_ms}, which
	identifies the time instant within the session at which an observation
	or status change is recorded.
	
	For this reason, a single session can be associated with multiple
	weather observations and multiple track-status records. Therefore,
	these tables cannot be inserted as normal static branches of the
	attribute tree: their values are determined not only by the session,
	but also by the corresponding time instant within the session.
	
	Instead, their information is transformed into derived contextual
	attributes whose grain is coherent with each fact. For the
	\textit{Lap Performance} fact, each lap is associated with the most
	recent valid contextual observation recorded in the same session at
	or before the lap completion time. Formally, for a lap $l$ and the set
	$C_l$ of contextual observations belonging to the same session, the
	selected record is:
	...
	c.\texttt{time\_ms}
	\;\middle|\;
	c.\texttt{time\_ms} \leq l.\texttt{time\_ms}
	\right\}.
	\]
	
	The selected weather values are subsequently transformed into
	categorical attributes, while the selected track-status record is
	mapped to its corresponding analytical category. For the
	\textit{Session Result} fact, valid weather observations are instead
	aggregated at session level, while track-status information is used
	only when relevant to the interpretation of the session outcome.
	
	The complete fallback rules, aggregation procedures, and
	classification thresholds are reported in
	Appendix~\ref{appendix:derived-attribute-rules}.
	...
	Table~\ref{tab:lap-derived-representative} reports the main derived attributes used in the edited \textit{Lap Performance} tree.The complete transformation rules and categorical thresholds are
	reported in Appendix~\ref{appendix:derived-attribute-rules}.
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.75\linewidth]{figures/result_focus.pdf}
	...
	After the editing of the attribute trees, the remaining attributes are translated into the multidimensional elements of the star schema. Direct dimensions are obtained from explicit nodes or branches of the edited attribute tree, while measures correspond to numerical attributes analysed at the grain of the fact.
	
	However, not every dimension originates from a single branch of the conceptual model. During the logical design, some dimensions are created by grouping multiple related attributes that describe the same analytical context. This transformation improves the organization of the multidimensional schema without changing the semantics of the underlying data.
	
	In this project, two cases are distinguished: contextual dimensions, which group related descriptive attributes into a single analytical context, and junk dimensions, which collect low-cardinality flags and quality indicators into dedicated dimensions, avoiding unnecessary fragmentation of the fact tables while supporting filtering and OLAP analysis.
	...
	For readability, Figure~\ref{fig:integrated-star-schema} provides an
	implementation-oriented overview of the two integrated star schemas.
	Only physical tables are represented as independent boxes.
	Degenerate dimensions are reported inside the corresponding fact
	table because they do not have a separate dimension table.
	
	The complete logical schemas, including attributes, primary keys,
	foreign keys, measures, and degenerate dimensions, are reported in
	Appendix~\ref{appendix:star-schema-details}.
	
	
	\vspace{4em}
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.90\linewidth]{figures/integrated_star_schema.png}
		\caption{Integrated logical star schema with shared and fact-specific dimensions.}
		\label{fig:integrated-star-schema}
	\end{figure}
	
	\noindent
	\textit{Note: DD denotes a degenerate dimension, namely a dimensional
		attribute stored directly in the corresponding fact table without a
		separate dimension table.}
	
	\newpage
	\section{Conclusion}
	
	This report documented the construction of a Formula 1 Data Warehouse starting from API-based data and manually created domain data.
	The raw data were first re-engineered into a reconciled PostgreSQL schema, then evaluated through data quality checks, missing-value analysis, outlier detection, and conservative cleaning rules.
	
	The cleaned data were used to design two complementary analytical facts: \textit{Lap Performance}, focused on lap-level performance, and \textit{Session Result}, focused on driver results at session level.
	The conceptual design produced two DFM fact schemas, while the logical design translated them into an integrated star schema with shared dimensions and specific analytical elements for each fact.
	
	The choice of the 2021 and 2022 seasons provides a useful comparison before and after the introduction of the 2022 technical regulations.
	Regulatory changes are particularly interesting because they can reduce the importance of previous technical advantages and change the competitive balance among teams.
	At the same time, a team that understands the new regulations more effectively from the beginning may gain useful knowledge and development experience for the following seasons of the same regulatory cycle.
	
	Although the warehouse contains data for all Formula 1 teams, the Tableau storytelling will focus on Ferrari, Red Bull, and Mercedes, which occupied the top three positions in the Constructors' standings in both seasons.
	This choice is an analytical scope decision and not a limitation of the warehouse, which can still support broader analyses involving all teams.
	
	The resulting warehouse provides a structured and OLAP-ready basis for the next phase of the project.
	...
	\begin{thebibliography}{9}
		...
		\bibitem{fia2022TechnicalRegulations}
		Fédération Internationale de l'Automobile.
		\textit{2022 Formula 1 Technical Regulations, Issue 11}.
		29 April 2022.
		Available at:
		\url{https://api.fia.com/sites/default/files/formula_1_-_technical_regulations_-_2022_-_iss_11_-_2022-04-29.pdf}.
		
		\bibitem{f12021vs2022}
		Formula 1 Official Website.
		\textit{2021 vs 2022: What's changed on Formula 1's all-new cars
			for this season}.
		22 February 2022.
		Available at:
		\url{https://www.formula1.com/en/latest/article/2021-vs-2022-whats-changed-on-formula-1s-all-new-cars-for-this-season.1SrJj7fQgv4Iw2IxCTnb1l}.
		
		\bibitem{therace}
		G. Anderson.
		\textit{Gary Anderson: What 2022-style F1 ground effect looks like}.
		The Race, 25 January 2022.
		Illustrations by Rosario Giuliana.
		Available at:
		\url{https://www.the-race.com/formula-1/gary-anderson-what-2022-style-f1-ground-effect-looks-like/}.
		
		\bibitem{fastf1docs}
		FastF1 Project.
		\textit{FastF1 Documentation}.
		Available at:
		\url{https://docs.fastf1.dev/}.
		
		\bibitem{jolpicaDocs}
		Jolpica F1 Project.
		\textit{Jolpica F1 API Documentation}.
		Available at:
		\url{https://github.com/jolpica/jolpica-f1/blob/main/docs/README.md}.
		...
		\bibitem{f1standings}
		Formula 1 Official Website.
		\textit{Formula 1 Drivers' and Constructors' Standings for the
	...
	\begin{longtable}{
		...
		& season\_year; grand\_prix\_id;\newline season\_year + round\_number; \newline grand\_prix\_id + session\_type; \newline session\_id + time\_ms. \\
		...
		& grand\_prix $\rightarrow$ season; \newline grand\_prix $\rightarrow$ circuit; \newline  session $\rightarrow$ grand\_prix; \newline lap/result $\rightarrow$ session, driver, team. \\
	...
	The main report presents only the pit-related cases and the missing \texttt{lap\_time\_ms} rule.
	...
	\begin{table}[H]
		...
		\begin{tabularx}{\linewidth}{p{5cm}p{3.2cm}p{3.2cm}X}
			\toprule
			\textbf{Condition}
			& \textbf{Missing attribute}
			& \textbf{Missing class}
	...
	\begin{lstlisting}
		...
		The Formula 1 data warehouse shows a very high general quality level. 
		...
		Some are explained by the Formula  domain, while others are suspicious and require review. 
	...
	\newpage
	\section{Additional Derived-Attribute Rules}
	\label{appendix:derived-attribute-rules}
	
	This appendix documents the deterministic rules used to transform
	raw or cleaned source attributes into the categorical and contextual
	attributes stored in the Data Warehouse.
	
	Unless otherwise stated, missing or unreliable source values produce
	the category \texttt{Unknown}. These rules are project-specific
	analytical conventions and do not represent official Formula 1
	classifications.
	
	
	\subsection{Driver Age Classification}
	
	Driver age is computed using the fixed reference date
	1 January 2022. A fixed date is used because the current
	\texttt{dim\_driver} contains one row per driver and does not
	implement a time-varying driver dimension.
	
	\[
	\text{ageYears} =
	\left\lfloor
	\frac{\text{2022-01-01} - \text{dateOfBirth}}
	{365.2425}
	\right\rfloor
	\]
	...
	\begin{table}[H]
		\centering
		\caption{Rules for \texttt{driver\_age\_class}.}
		\begin{tabularx}{0.80\linewidth}{p{3.5cm}X}
			\toprule
			\textbf{Class} & \textbf{Rule} \\
			\midrule
			\texttt{Young} & $\text{ageYears} < 25$ \\
			\texttt{Intermediate} & $25 \leq \text{ageYears} < 30$ \\
			\texttt{Senior} & $\text{ageYears} \geq 30$ \\
			\texttt{Unknown} & Date of birth missing or unreliable \\
			\bottomrule
		\end{tabularx}
	\end{table}
	
	
	\subsection{Tyre-Context Classification}
	
	\begin{table}[H]
		\centering
		\caption{Rules for tyre-context derived attributes.}
		\begin{tabularx}{\linewidth}{p{5.5cm}p{4cm}X}
			...
			\texttt{starting\_tyre\_set\_status}
			& \texttt{New}
			& \texttt{fresh\_tyre = TRUE} \\
			
			\texttt{starting\_tyre\_set\_status}
			& \texttt{Used}
			& \texttt{fresh\_tyre = FALSE} \\
			
			\texttt{starting\_tyre\_set\_status}
			& \texttt{Unknown}
			& \texttt{fresh\_tyre} is missing or unreliable \\
			
			\midrule
			
			\texttt{tyre\_life\_class}
			& \texttt{Low}
			& $0 \leq \texttt{tyre\_life} \leq 5$ \\
			
			\texttt{tyre\_life\_class}
			& \texttt{Medium}
			& $5 < \texttt{tyre\_life} \leq 15$ \\
			...
			\texttt{tyre\_life\_class}
			& \texttt{Unknown}
			& Value missing, negative, or unreliable \\
	...
	\begin{table}[H]
		\centering
		\caption{Thresholds for weather-derived classes.}
		\begin{tabularx}{\linewidth}{p{4cm}p{3cm}X}
			\toprule
			\textbf{Attribute} & \textbf{Class} & \textbf{Rule} \\
			\midrule
			
			\texttt{air\_temp\_class}
			& \texttt{Low}
			& $\texttt{air\_temp} < 20\,^{\circ}\mathrm{C}$ \\
			
			\texttt{air\_temp\_class}
			& \texttt{Medium}
			& $20 \leq \texttt{air\_temp} < 28\,^{\circ}\mathrm{C}$ \\
			
			\texttt{air\_temp\_class}
			& \texttt{High}
			& $\texttt{air\_temp} \geq 28\,^{\circ}\mathrm{C}$ \\
			
			\midrule
			
			\texttt{track\_temp\_class}
			& \texttt{Low}
			& $\texttt{track\_temp} < 30\,^{\circ}\mathrm{C}$ \\
			
			\texttt{track\_temp\_class}
			& \texttt{Medium}
			& $30 \leq \texttt{track\_temp} < 42\,^{\circ}\mathrm{C}$ \\
			
			\texttt{track\_temp\_class}
			& \texttt{High}
			& $\texttt{track\_temp} \geq 42\,^{\circ}\mathrm{C}$ \\
			
			\midrule
			
			\texttt{wind\_speed\_class}
			& \texttt{Low}
			& $\texttt{wind\_speed} < 2$ \\
			
			\texttt{wind\_speed\_class}
			& \texttt{Medium}
			& $2 \leq \texttt{wind\_speed} < 5$ \\
			
			\texttt{wind\_speed\_class}
			& \texttt{High}
			& $\texttt{wind\_speed} \geq 5$ \\
			
			\bottomrule
		\end{tabularx}
	\end{table}
	
	For each weather attribute, the value \texttt{Unknown} is assigned
	when the corresponding numerical observation is missing or has been
	marked as unreliable during Data Quality Assessment or Data Cleaning.
	...
	\vspace{2em}
	This appendix section reports full-size versions of the star schema diagrams.
	
	\vspace{2em}
	\noindent\textit{Note:} In the star schema diagrams, \texttt{DD} denotes a degenerate dimension included directly in the corresponding fact table.
