...
\begin{document}
	...
	\begin{figure}[H]
		...
		\begin{subfigure}{0.48\linewidth}
			\centering
			\includegraphics[width=1\linewidth]{figures/missing_class_distribution.pdf}
		...
		\begin{subfigure}{0.48\linewidth}
			\centering
			\includegraphics[width=1\linewidth]{figures/missing_information_area.pdf}
	...
	\subsection{LLM-supported Interpretation of Data Quality Outputs}
	...
	The input included the outputs of the General DQA checks, the focused missing-value analysis, and the outlier detection procedure.
	...
	This step was used as an additional documentation and interpretation layer, useful to translate technical quality outputs into readable explanations. 
	...
	\url{https://github.com/GiuseppeRudi/ground_effect-dw}
	...
	\subsection{LLM-supported Data Quality Interpretation}
	\label{appendix:llm-dqa-interpretation}
	
	This appendix reports the prompt used to support the interpretation of the Data Quality Assessment outputs through a Large Language Model. 
	The LLM was not used to perform the official DQA, clean the data, or make automatic decisions. 
	Its role was limited to generating a readable interpretation of deterministic artefacts already produced by the project scripts.
	
	\subsubsection*{Prompt provided to the LLM}
	
	The following prompt was used to instruct the LLM about its role, the available inputs, the expected output structure, and the constraints to follow.
	
	\begin{lstlisting}
		% Insert here the full prompt used for the LLM-supported interpretation.
		% The prompt includes:
		% - the role of the assistant;
		% - the available deterministic DQA artefacts;
		% - the required Markdown sections;
		% - the instruction not to invent values or hidden causes;
		% - the rule that all suggested actions must be manually reviewed.
	\end{lstlisting}
