...
\begin{document}
	...
	The five assessed ISO-oriented dimensions are:
	...
	% -------------------------------------------------------
	\paragraph{Timeliness exclusion}
	% -------------------------------------------------------
	
	Timeliness is not scored as a separate quality dimension because the dataset contains
	closed historical seasons and has no freshness requirement. Temporal rules, including
	date ordering and within-session timelines, are assessed under Consistency.
	...
	The assessment script produces the following outputs:
	...
	\begin{longtable}{p{7cm}p{9cm}}
		...
		\midrule
		\endhead
		\texttt{dq\_check\_results.csv} & Detailed result with one row for every executed table-level check. \\
		\midrule
		\texttt{dq\_scorecard\_by\_table.csv} & Summary score by table, useful to identify the weakest tables. \\
		\midrule
		\texttt{issues\_all\_tables.csv} & Consolidated row-level issues detected across the reconciled schema. \\
		\midrule
		\texttt{issues\_lap.csv}, \texttt{issues\_result.csv}, \texttt{issues\_track\_status.csv} & Per-table issue extracts generated for the tables requiring row-level investigation. \\
		\midrule
		\texttt{referential\_integrity\_issues.csv} & Broken references between child and parent tables. \\
		\bottomrule
	\end{longtable}
	...
	This section documents the checks executed on each table of the reconciled database.
	...
	\begin{longtable}{p{3.5cm}p{11cm}}
		...
		Completeness
		& \texttt{circuit\_id}, \texttt{circuit\_name}, \texttt{country},
		\texttt{location}, \texttt{global\_circuit\_category},
		\texttt{sector1\_category}, \texttt{sector2\_category}, and
		\texttt{sector3\_category} must be present. These attributes are required
		because they identify, describe, and classify the circuit and are later embedded
		in the physical \texttt{dim\_grand\_prix}. \\
		\midrule
	...
	\begin{longtable}{p{3.5cm}p{11cm}}
		...
		or equal to zero. Missing \texttt{grid\_position} is allowed for Qualifying;
		in Race it is recorded as a race-context information issue rather than replaced
	...
	The following figures are generated from the actual DQA, focused-missing, and outlier
	outputs. They replace the former placeholders and summarize the evidence consumed by
	the cleaning phase.

	\begin{figure}[H]
		\centering
		\includegraphics[width=0.94\textwidth]{../final_report/figures/dqa_dimension_heatmap.pdf}
		\caption{Executed checks and scores by reconciled table and assessed quality dimension.}
		\label{fig:dq-score-heatmap}
	\end{figure}

	\begin{figure}[H]
		\centering
		\includegraphics[width=0.88\textwidth]{../final_report/figures/missing_class_distribution.pdf}
		\caption{Observed missing-value classes produced by the focused analysis.}
		\label{fig:dq-missing-class-distribution}
	\end{figure}

	\begin{figure}[H]
		\centering
		\includegraphics[width=0.92\textwidth]{../final_report/figures/missing_information_area.pdf}
		\caption{Missing-information areas used to derive row-level quality flags.}
		\label{fig:dq-missing-information-area}
	\end{figure}

	\begin{figure}[H]
		\centering
		\includegraphics[width=0.92\textwidth]{../final_report/figures/outlier_methods_by_metric.pdf}
		\caption{Outlier detections by metric and method, before consensus-based cleaning.}
		\label{fig:dq-outlier-methods}
	\end{figure}
	...
	\begin{itemize}
		\item \texttt{outlier\_summary.csv}, aggregated by metric;
		\item \texttt{lap\_outlier\_flags.csv}, with row-level flags;
		\item a consensus score for suspicious values;
		\item plots showing outlier distributions;
		\item deterministic evidence consumed by the cleaning policy.
	\end{itemize}

	Detection and cleaning remain separate. In particular, the detector never deletes a
	record. During cleaning, consensus 2 on \texttt{lap\_time\_ms} triggers
	\texttt{DROP\_ROW}, whereas sector and speed outliers preserve the lap and activate
